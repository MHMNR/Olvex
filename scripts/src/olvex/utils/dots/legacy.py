...
from olvex.utils.paths import config_dir, data_dir

LEGACY_META_PKG = "olvex-meta"
...
def _find_legacy_repo(path: Path) -> Path | None:
    ...
    # Check remote
    if remote.strip() != "https://github.com/olvex-dots/olvex.git":
        return
...
def detect_legacy_repo() -> Path | None:
    ...
    return _find_legacy_repo(data_dir / "olvex")
...
def legacy_symlinks(legacy_dir: Path | None) -> list[Path]:
    ...
    extras = [
        *(Path.home() / ".zen").glob("*/chrome/userChrome.css"),
        Path.home() / ".local/lib/olvex/olvexfox",
    ]
