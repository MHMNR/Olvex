...
def print_version() -> None:
    ...
    print()
    try:
        olvex_dir = (config_dir / "hypr").resolve().parent
        olvex_ver = subprocess.check_output(
            ["git", "--git-dir", olvex_dir / ".git", "rev-list", "--format=%B", "--max-count=1", "HEAD"], text=True
        )
        print("Olvex:")
        print("    Last commit:", olvex_ver.split()[1])
        print("    Commit message:", *olvex_ver.splitlines()[1:])
    except subprocess.CalledProcessError:
        print("Olvex: not installed")

    print()
    try:
        shell_ver = subprocess.check_output(["/usr/lib/olvex/version", "-s"], text=True).strip()
        print("Shell:")
        print("    ", shell_ver)
    ...
    local_shell_dir = config_dir / "quickshell/olvex"
