...
from olvex.utils.colour import get_dynamic_colours
from olvex.utils.hypr import is_lua_config
from olvex.utils.io import log_exception
...
def apply_discord(scss: str) -> None:
    ...
    for client in "Equicord", "Vencord", "BetterDiscord", "equibop", "vesktop", "legcord":
        atomic_write(config_dir / client / "themes/olvex.theme.css", conf)
...
def apply_pandora(colours: dict[str, str], mode: str) -> None:
    ...
    atomic_write(data_dir / "PandoraLauncher/themes/olvex.json", template)


@log_exception
def apply_spicetify(colours: dict[str, str], mode: str) -> None:
    template = gen_replace(colours, templates_dir / f"spicetify-{mode}.ini")
    atomic_write(config_dir / "spicetify/Themes/olvex/color.ini", template)
...
def apply_btop(colours: dict[str, str]) -> None:
    ...
    atomic_write(config_dir / "btop/themes/olvex.theme", template)
...
def apply_qt(colours: dict[str, str], mode: str, icon_theme: str | None = None) -> None:
    ...
    atomic_write(config_dir / "qtengine/olvex.colors", colours)
...
def apply_warp(colours: dict[str, str], mode: str) -> None:
    ...
    atomic_write(data_dir / "warp-terminal/themes/olvex.yaml", template)
...
def apply_chromium(colours: dict[str, str]) -> None:
    ...
    for cmd, policy_dir in browsers:
        ...
        subprocess.run(
            ["sudo", "-n", "tee", str(policy_dir / "olvex.json")],
...
def apply_zed(colours: dict[str, str], mode: str) -> None:
    theme_path = config_dir / "zed/themes/olvex.json"
