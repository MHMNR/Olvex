...
public:
    ...
    Q_INVOKABLE void copy(const QString &entry) {
        ...
        const QString cmd = QStringLiteral("printf '%%s' '%1' | cliphist %2decode | wl-copy")
    ...
    Q_INVOKABLE void copyText(const QString &text) {
        ...
        const QString cmd = QStringLiteral("printf '%%s' '%1' | wl-copy").arg(escaped);
        runBash(cmd);
        scheduleRefresh();
    }
    ...
    Q_INVOKABLE void deleteEntry(const QString &entry) {
        ...
        const QString escaped = shellSingleQuoteEscape(entry);
        const QString cmd = QStringLiteral("printf '%%s' '%1' | cliphist %2delete")
                                .arg(escaped, cliphistDbShellPrefix());
        runBash(cmd);
        refresh();
    }
