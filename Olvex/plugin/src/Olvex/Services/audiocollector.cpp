...
Q_LOGGING_CATEGORY(lcAc, "olvex.services.ac", QtInfoMsg)
Q_LOGGING_CATEGORY(lcAcWorker, "olvex.services.ac.worker", QtInfoMsg)
...
void AudioCollector::loadChunk(const qint16* samples, quint32 count) {
    ...
    // qCDebug(lcAc) << "AudioCollector: loading" << count << "samples, max amp:" << *std::max_element(samples, samples + count);
...
void AudioCollector::start() {
    qCDebug(lcAc) << "AudioCollector::start: spawning thread";
