...
class Tokens : public QQuickAttachedPropertyPropagator, public QQmlParserStatus {
    ...
    Q_PROPERTY(const olvex::config::AppearanceRounding* rounding READ rounding NOTIFY sourceChanged)
    Q_PROPERTY(const olvex::config::AppearanceSpacing* spacing READ spacing NOTIFY sourceChanged)
    Q_PROPERTY(const olvex::config::AppearancePadding* padding READ padding NOTIFY sourceChanged)
    Q_PROPERTY(const olvex::config::AppearanceFont* font READ font NOTIFY sourceChanged)
    Q_PROPERTY(const olvex::config::AppearanceTransparency* transparency READ transparency NOTIFY sourceChanged)
    Q_PROPERTY(const olvex::config::SizeTokens* sizes READ sizes NOTIFY sourceChanged)
    Q_PROPERTY(const olvex::config::AnimTokens* anim READ anim NOTIFY sourceChanged)
