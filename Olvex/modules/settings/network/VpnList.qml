...
import qs.services
    ...
    Connections {
        function onConnectedChanged() {
            if (!VPN.connected && root.pendingSwitchIndex >= 0) {
                ...
                const providers = [];
                for (let i = 0; i < GlobalConfig.qspanel.vpn.provider.length; i++) {
                    const p = GlobalConfig.qspanel.vpn.provider[i];
                    if (typeof p === "object") {
                        const newProvider = {
                            name: p.name,
                ...
                GlobalConfig.qspanel.vpn.provider = providers;
    ...
    StyledListView {
        ...
        interactive: false
        edgeFades: false // non-scrolling packed list
        spacing: Tokens.spacing.smaller

        model: ScriptModel {
            values: GlobalConfig.qspanel.vpn.provider.map((provider, index) => {
    ...
    Popup {
        ...
        function showEditForm(index: int): void {
            const provider = GlobalConfig.qspanel.vpn.provider[index];
            const isObject = typeof provider === "object";
            ...
            ColumnLayout {
                ...
                TextButton {
                    ...
                    onClicked: {
                        const providers = [];
                        for (let i = 0; i < GlobalConfig.qspanel.vpn.provider.length; i++) {
                            providers.push(GlobalConfig.qspanel.vpn.provider[i]);
                        }
                        providers.push({
                            name: "netbird",
                            displayName: "NetBird",
                    ...
                    TextButton {
                        ...
                        onClicked: {
                            ...
                            if (vpnDialog.editIndex >= 0) {
                                const oldProvider = GlobalConfig.qspanel.vpn.provider[vpnDialog.editIndex];
