...
Item {
    ...
    focus: true
    Keys.onEscapePressed: root.visibilities.powermenu = false
    Component.onCompleted: root.forceActiveFocus()

    // Background handled by system blur / ContentWindow scrim
    MouseArea {
        anchors.fill: parent
        onClicked: root.visibilities.powermenu = false
    }
    ...
    Row {
        ...
        SessionColumn {
            id: hibernateBtn
            width: (mainRow.width / root.totalWeight) * weight
            icon: Config.powermenu.icons.hibernate
            label: "SLEEP"
            command: Config.powermenu.commands.hibernate
            hue: "Primary"
    ...
    component SessionColumn: Item {
        ...
        Keys.onPressed: event => {
            if (!Config.powermenu.vimKeybinds) return;
