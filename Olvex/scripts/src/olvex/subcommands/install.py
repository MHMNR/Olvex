...
from olvex.utils.dots.manifest import ComponentError, Manifest, ManifestError
from olvex.utils.dots.misc import build_local_packages, run_hooks
from olvex.utils.dots.packages import DEFAULT_AUR_HELPER, PackageError, PackageInstaller
from olvex.utils.dots.source import DotsSource, SourceError
from olvex.utils.dots.state import DotsState
from olvex.utils.io import confirm, disable_input, fatal, info, log, pause, prompt_selection, warn
from olvex.utils.paths import (
    config_backup_dir,
    config_dir,
)
...
class Command:
    ...
    def print_greeting(self) -> None:
        print(
            "\033[38;2;150;241;241m"  # Olvex colour
            + textwrap.dedent(
                r"""
                ╭─────────────────────────────────────────────────╮
                │      ______           __          __  _         │
    ...
    def migrate_legacy(self, installer: PackageInstaller, legacy_dir: Path | None) -> None:
        ...
        print()
        log("Found a legacy Olvex installation...")
    ...
    def print_done(self) -> None:
        print()
        info("All done! Olvex has been installed.")
        info("A few things to finish up:")
        info("  - A reboot is recommended for all changes take effect")
        info("  - Edit `~/.config/olvex/hypr-vars.conf` to set default apps, keybinds and much more")
        info("  - Edit `~/.config/olvex/hypr-user.conf` to set your monitor layout and other Hyprland configs")
        info("  - Run `olvex update` later to pull in the latest changes")
