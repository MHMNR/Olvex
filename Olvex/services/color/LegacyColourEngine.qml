...
Item {
    ...
    function load(data: string, isPreview: bool): void {
        ...
        if (!scheme) {
            console.log("[Colours] No valid scheme in payload");
            return;
        }
        console.log(`[Colours] Loading ${isPreview ? "preview" : "current"} palette (${Object.keys(scheme.colours ?? {}).length} roles)`);
        const colours = isPreview ? preview : current;

        if (isPreview) {
            const mode = GlobalConfig.appearance.themeMode;
            previewLight = (mode === "auto") ? (scheme.mode === "light") : (mode === "light");
        } else {
            root.scheme = scheme.name;
            root.flavour = scheme.flavour;
            root.schemeMode = scheme.mode;
        }

        Mapper.applySchemeToPalette(colours, scheme);
        console.log("[Colours] Palette update complete");

        if (!isPreview) {
            const mode = GlobalConfig.appearance.themeMode;
            if (mode !== "auto" && scheme.mode !== mode) {
                console.log(`[Colours] Mode mismatch (${scheme.mode} vs ${mode}), syncing`);
                root.setMode(mode);
            }
        }
    }
    ...
    ImageAnalyser {
        id: analyser
        // ImageAnalyser's C++ backend uses QImage, which strictly requires a raw system path, not a URL
